...
public class CSVFile {
    ...
    public void setFeatureSchema() throws IOException, CSVFileException {
        ...
        if (!initialized) init();
        try (InputStream in = CompressedFile.openFile(getFilePath(), getEntryName());
    ...
    public Feature getFeature(String line) throws ParseException {
        ...
        for (int i = 0 ; i < fields.length ; i++) {
            ...
            // geometryColumns is an array containing indexes of geometry columns
            for (int j : geometryColumns) {
                if (j == i) {
                    geom = true; 
                    break;
                }
    ...
    public String toString() {
        ...
        if (schema != null) {
            for (int i = 0 ; i < schema.getAttributeCount() ; i++) {
                sb.append("        ")
                    .append(schema.getAttributeName(i))
                    .append("(")
                    .append(schema.getAttributeType(i))
                    .append(")\n");
            }
        }
        sb.append("}");
